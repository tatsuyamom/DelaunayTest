...
void ofxDelaunay::drawTriangles(ofImage img){
	for(int i = 0; i < ntri; i++){
        int index = (p[v[i].p1].y+p[v[i].p2].y+p[v[i].p3].y)/3 * img.width + ((p[v[i].p1].x+p[v[i].p2].x+p[v[i].p3].x)/3);
        int valR, valG, valB, valA;
        valR = img.getPixels()[index * 3];
        valG = img.getPixels()[index * 3 + 1];
        valB = img.getPixels()[index * 3 + 2];
        
        ofSetColor(valR,valG,valB);
        ofFill();
